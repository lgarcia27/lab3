...
#######Red Black Trees code implementation########################
class RedBlackTree:
    def __init__(self):
        self.root = None
...
##################Anagram Implementation ###################################
    ...
    def menu_Res():
        print("Would you like to use AVL which is 1 or Red-Black Tree which is 2")
        print("If no selection then I'll see you later")
        print("Bye-Bye...")        
    ...
    def count_anagrams(word, prefix=""):
        ...
        if len(word) <= 1:
            ...
            for i in range(len(word)):
                current = word[i: i + 1]
                bef = word[0: i]  # letters before cur
                aft = word[i + 1:]  # letters after cur
    
                if current not in bef:  # Check if permutations of cur have not been generated.
                    count += count_anagrams(bef + aft, prefix + current)
